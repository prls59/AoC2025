...
for a in range(len(box_list) -1):
    for b in range(a + 1, len(box_list)):
...
for c in range(connections):
    circuit_id = connect(nearby_boxes[c][0], nearby_boxes[c][1], circuit_id, circuit_size)

# get check product
circuit_list = sorted(circuit_size, key=lambda k: circuit_size[k], reverse=True)
result = 1
for n in range(check):
    result *= circuit_size[circuit_list[n]]
